...
parser.py
─────────
Lê logs brutos da VM, aplica o Drain3 e salva:
  - data/parsed/templates.json   → mapeamento cluster_id → template
  - data/parsed/<nome>.eventids  → sequência de IDs por arquivo de log
"""
...
# Adiciona utils/Drain3 ao path para importar drain3 local
...
# ── Configuração do Drain3 via API (sem arquivo .ini) ──────────────────────
...
def _build_drain_config() -> TemplateMinerConfig:
    ...
    # Converte os dicts de config.py em objetos MaskingInstruction
...
# ── Extração da mensagem útil do syslog ───────────────────────────────────

_SYSLOG_RE = re.compile(SYSLOG_REGEX)

def extract_message(raw_line: str) -> str:
    """Remove timestamp/hostname/processo e retorna só a mensagem."""
...
class LogParser:
    def __init__(self):
        ...
        self.templates: dict[int, str] = {}  # cluster_id → template string

    def parse_file(self, log_path: str) -> list[int]:
        """
        Processa um arquivo de log linha a linha.
        Retorna a sequência de event IDs (cluster IDs do Drain3).
        """
        event_ids: list[int] = []
    ...
    def parse_directory(self, directory: str) -> dict[str, list[int]]:
        """Processa todos os .log de um diretório."""
        results = {}
    ...
    def save_results(self, sequences: dict[str, list[int]]):
        """Salva templates e sequências de event IDs em data/parsed/."""
        # Templates
    ...
    def print_templates(self):
        """Imprime resumo dos templates encontrados."""
        print(f"\n{'─'*60}")
